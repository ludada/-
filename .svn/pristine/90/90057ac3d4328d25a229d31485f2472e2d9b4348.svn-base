...
@property (assign, nonatomic, getter = isExecuting) BOOL executings;
@property (assign, nonatomic, getter = isFinished) BOOL finisheds;
...
- (void)cancel
{
    ...
    if (self.connection)
    {
        ...
        if (self.isExecuting) self.executings = NO;
        if (!self.isFinished) self.finisheds = YES;
    }
...
- (void)setFinisheds:(BOOL)finisheds
{
    [self willChangeValueForKey:@"isFinished"];
    _finisheds = finisheds;
    [self didChangeValueForKey:@"isFinished"];
}

- (void)setExecutings:(BOOL)executings
{
    [self willChangeValueForKey:@"isExecuting"];
    _executings = executings;
