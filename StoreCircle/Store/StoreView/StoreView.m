...
- (void)createMyScrollView
{
    ...
    if (iPhone4) {
        myView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 100);
    }
...
{
    self.myView1 = [[UIButton alloc] initWithFrame:CGRectMake(5, self.scroll.frame.origin.y + self.scroll.frame.size.height + 5, SCREEN_WIDTH - 10, 30)];
    [self.myView1 setImage:[UIImage imageNamed:@"login"] forState:UIControlStateNormal];
    self.myView1.layer.cornerRadius = 7;
    self.myView1.layer.masksToBounds = YES;
    [self addSubview:self.myView1];
    
//    NSArray *array = [NSArray arrayWithObjects:@"外卖", @"餐饮娱乐", @"便民服务", @"宾馆洗浴", @"其他", nil];
    for (int i = 0; i < self.typeArray.count; i++) {
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(self.myView1.frame.size.width/5 * i, 0, self.myView1.frame.size.width/5, self.myView1.frame.size.height)];
...
//        [button.titleLabel setFont:[UIFont boldSystemFontOfSize:15]];
        ...
        button.tag = i + 100;
        [self.myView1 addSubview:button];
        if (i == 0) {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.scroll.frame.origin.y + self.scroll.frame.size.height + 40, SCREEN_WIDTH, SCREEN_HEIGHT - (self.scroll.frame.origin.y + self.scroll.frame.size.height + 40 + 64 + 49)) style:UITableViewStylePlain];
