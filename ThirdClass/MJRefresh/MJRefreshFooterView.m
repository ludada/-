...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state)
    {
		case MJRefreshStateNormal:
        {
            ...
            NSInteger currentCounts = [self totalDataCountInScrollView];
            // 刚刷新完毕
            if (MJRefreshStateRefreshing == oldState && deltaH > 0 && currentCounts != self.lastRefreshCount) {
...
- (NSInteger)totalDataCountInScrollView
{
    ...
    if ([self.scrollView isKindOfClass:[UITableView class]]) {
        ...
        for (NSInteger section = 0; section<tableView.numberOfSections; section++) {
            totalCounts += [tableView numberOfRowsInSection:section];
        }
    } else if ([self.scrollView isKindOfClass:[UICollectionView class]]) {
        UICollectionView *collectionView = (UICollectionView *)self.scrollView;
        
        for (NSInteger section = 0; section<collectionView.numberOfSections; section++) {
            totalCounts += [collectionView numberOfItemsInSection:section];
        }
    }
    return totalCounts;
}
